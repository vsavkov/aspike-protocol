...
remove(Address, Port, User, Encrypted_password, Namespace, Set, Key) ->
  case aspike_protocol:open_session(Address, Port, User, Encrypted_password) of
    ...
    {ok, #{socket := Socket} = Session} ->
      ...
      Encoded = aspike_protocol:enc_remove_request(Namespace, Set, Key_digest),
      ...
      case aspike_protocol:receive_response_message(Socket, 1000) of
        ...
        Response ->
          aspike_protocol:close_session(Session),
          case aspike_protocol:dec_remove_response(Response) of
            need_more ->
